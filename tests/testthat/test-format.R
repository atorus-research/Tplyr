...
# TODO: Fix to use the non-distinct count
test_that("Hug character formatting applies properly for count layers", {
  ...
  # Multi hug character, count layer, manual
  x <- tplyr_table(adsl, TRT01P) %>%
    add_layer(
      group_count(RACE) %>%
        set_format_strings(f_str("a {(XXX.x%)}", n, pct))
  ...
  x <- tplyr_table(adsl, TRT01P) %>%
    add_layer(
      group_count(RACE) %>%
        set_format_strings(f_str("a (A.x%)", n, pct))
    ) %>%
    build() %>%
    pull(var1_Placebo)
