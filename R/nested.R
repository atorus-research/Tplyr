...
process_nested_count_target <- function(x) {
  ...
  evalq({
    ...
    #   first_var_length <- length(levels(target[[as_name(target_var[[1]])]]))
    #   second_var_length <- length(levels(target[[as_name(target_var[[2]])]]))
    #
    #   assert_that(second_var_length >= first_var_length,
    #               msg = "The number of levels of your second variable must be greater than the number of levels in your first variable")
    # }
