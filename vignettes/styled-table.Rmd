...
adsl <- adsl %>% 
  mutate(
    ...
    RACE = factor(RACE, c("AMERICAN INDIAN OR ALASKA NATIVE", "ASIAN", "BLACK OR AFRICAN AMERICAN", 
                          "NATIVE HAWAIIN OR OTHER PACIFIC ISLANDE", "WHITE", "MULTIPLE"))
  )
...
- The second pre-processing step does something interesting. If you recall from the Sorting vignette, factor variables input to 'Tplyr' will use the factor order for the resulting order variable. Another particularly useful advantage of this is dummying values. The adsl dataset only contains the races "WHITE", "BLACK OR AFRICAN AMERICAN", and "AMERICAN INDIAN OR ALASK NATIVE". If you set factor levels prior to entering the data into 'Tplyr', the values will be dummied for you. This is particularly advantageous when a study is early on and data may be sparse. Your output can display complete values and the presentation will be consistent as data come in. 
...
- Sorting the data by the layer, by the row labels (which in this case are just text strings we defined), and by the results. Review the Sorting vignette to understand how each layer handles sorting in more detail.
- The next step is where I re-order the columns and drop off the order columns. For more information about how to get the most out of `dplyr::select`, you can look into 'tidyselect' [here](https://tidyselect.r-lib.org/reference/select_helpers.html). This is where the `starts_with` comes.
- The last step is a 'Tplyr' function. Essentially, **after** your data are sorted, this function will look at all of your `row_label` variables and drop any repeating values. For packages like 'huxtable', this eases the process of making your table presentation ready, so you don't need to do any cell merging once the 'huxtable' table is created.
- Added the column headers. In a huxtable, your column headers are basically just the top rows of your data frame. `add_column_headers()` does this for you by letting you just enter in a string to define your headers. But there's more to it - you can also create nested headers by nesting text within curly brackets ({}), and notice that we have the treatment groups within the two stars? This actually allows you to take the header N values that Tplyr calculates for you, and use it within the column headers. As you can see in the first row of the output, the text shows the (N=XX) values populated with the proper header_n counts.
...
This may look a little messy, but pharmaRTF syntax can be abbreviated by standardizing a process. Check out [this vignette](https://atorus-research.github.io/tf_from_file.html) for instructions on how to read titles and footnotes into pharmaRTF from a file. 
