...
Note how the entire example above has all records in `var1_Placebo` missing, as well as some in `var1_Xanomeline High Dose`. 'Tplyr' gives you control over how you fill this space. Let's say that we wanted instead to make that space say "Missing". You can control this with the `f_str()` object using the `empty` parameter
