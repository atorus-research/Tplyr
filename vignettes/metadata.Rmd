...
```{r add_vars}
get_meta_subset(t, 'c2_1', 'var1_Placebo', add_cols = quos(USUBJID, SEX)) %>% 
  kable()
```

Variables should be provided using `rlang::quos()`, just like the `cols` argument on `tplyr_table()` and the `by` arguments in each layer type. 
