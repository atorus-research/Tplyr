...
Counting is pretty easy, right? There's not all that much to it. With of a few considerations, we can cover most of the scenarios that users will encounter while using Tplyr. Denominators, on the other hand, get *_a lot_* more complicated. Why? Because there are a lot of ways to do it. What values do we exclude from the denominator? What groups of variables establish thought groups? Does the denominator use a different filter than the values being counted? If you're programmed enough of these tables, you know that it's all very situational. 

Given the complexity, we've spent a lot of time trying to make this easy for you, the user. And it became very clear that denominators needed their own vignette. Additionally, a few other things go hand in hand with denominators. 
...
There are three things done above that trigger the use of the population data:
  - Population data must be set in the first place
  ...
  - The population denominator is then accessed using `distinct_pct`. Also, it's worth noting that the default count layer formats use distinct values.
...
When you're looking within a single dataset, there are a couple factors that you need to consider for a denominator. The first is which grouping variables create those denominators. Let's look at this from two perspective - count layers and shift layers.
...
Ok - so, now this is a little bit off. By breaking sex down as a row group, the denominators are still the total treatment group. Does that make sense? 34 female Placebo group subjects completed, but that 39.5% includes male subjects in the denominator as well. Let's fix this using `set_denoms_by()`.
...
tplyr_table(adlb, TRTA, where=PARAMCD == "CK") %>%
  add_layer(
    group_shift(vars(row = BNRIND, column = ANRIND), by = vars(PARAM, AVISIT)) %>%
      set_format_strings(f_str("xx (xxx.x%)", n, pct)) %>%
      set_denoms_by(BNRIND, PARAM, AVISIT) # Each row made by BNRIND, TRTA
  ) %>%
  build() %>%
...
Let's look at a more practical version of the table above. Typically you would want the total count to be your denominator, which excludes the missings. So more commonly, you'll see this:
...
Now the denominators reflect the total counts, and we can see how many records are excluded from the presented group.
