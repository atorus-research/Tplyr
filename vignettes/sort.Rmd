...
```{r}
byvarn_t <- tplyr_table(adsl, TRT01A) %>%
  add_layer(
...
byfact_t <- tplyr_table(adsl, TRT01A) %>%
  add_layer(
...
```{r}
bycount_t <- tplyr_table(adsl, TRT01A, cols = AGEGR1) %>%
  add_layer(
    group_count(EOSSTT) %>%
...
`set_result_order_var()` can also be used when there are distinct counts. In the example `ord_layer_1` is presented with the distinct counts rather than the event counts.

```{r}
adae_t <- tplyr_table(adae, TRTA) %>%
  set_pop_data(adsl) %>%
...
```{r}
nested <- tplyr_table(adsl, TRT01A) %>%
  add_layer(
