...
One sort method can be passed which will be used by both variables. In this example both `EOSSTT` and `DCDECOD` will be sorted by the count in the 'Total' column.
...
If multiple types of counts are used in the layer, `set_numeric_result_order_var()` can be used to select which should be used in ordering.
...
`set_result_order_var()` can also be used when there are multiple numbers presented in the summary within one cell. In the example `ord_layer_1` is presented with the distinct counts rather than the event counts.
