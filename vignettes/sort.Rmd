...
By default the ordering helper for the grouping rows is set to Inf to ensure they are always at the top after being sorted. If you need to sort the outer rows on the bottom, or sort the other values in a descending order, the function `set_outer_inf()` allows you to change this behavior and set the ordering helper to -Inf.
