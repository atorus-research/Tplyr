...
Within `use_template()`, the first parameter is the template name. After that, we supply arguments as we normally would into `group_count()`, `group_desc()`, or `group_shift()`. Additionally, note that our formats have been applied just as they would be if we used `set_format_strings()` as specified in the template. Our template was a applied, the table built with all of the settings appropriately. 
...
If you want to view any available templates in your session, use the function `view_layer_templates()`.
