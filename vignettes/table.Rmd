...
In addition to `treat_var`, `tplyr_table()` has two additional parameters:

- `where`: The overarching table subset criteria. Each layer will use this subset by default. The `where` parameter at the table level will be called **in addition to** this subset criteria.
...
We've added the function `add_treat_grps()` to do this work for you. Additionally, to simplify the process we added an abstraction of `add_treat_grps()` named `add_total_group()` to simplify the process of creating a "Total" group.
...
'Tplyr' allows you to provide a separate population dataset to overcome this. Furthermore, you are also able to provide a separate population dataset `where` parameter, as the variable names may differ between the datasets.
...
In the above example, setting the population dataset `where` clause to `TRUE` removes any filter. `AEREL` doesn't exist in ADSL, therefore we removed the filter criteria on the population data. If `set_pop_where()` is not set for the population data, it will default to the `where` parameter used in `tplyr_table()`. The same logic applies to the population treatment variable. `TRTA` does not exist in `adsl` either, so we used `set_pop_treat_var()` to change it to the appropriate variable in `adsl`. 
