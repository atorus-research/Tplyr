...
shf <- group_shift(t, quos(row=COMP8FL, column=COMP24FL))
shf
...
tplyr_table(adsl, TRT01P) %>% 
  add_layer(
    group_desc(quos(AGE, AVGDD), by = "Age and Avg. Daily Dose")
  ) %>% 
  build() %>% 
  kable()
...
Here we again use `rlang::quos()` to specify multiple target variables. When used in a count layer, **Tplyr** knows automatically that the first variable is a grouping variable for the second variable, and counts shall be produced for both then merged together.
...
tplyr_table(adlb, TRTA, where = PARAMCD == "CK") %>%
  add_layer(
    group_shift(quos(row=BNRIND, column=ANRIND), by=PARAM) %>% 
