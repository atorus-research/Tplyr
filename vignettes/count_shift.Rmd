...
## Controlling Denominator Where Clause

While `set_denom_ignore` can be useful for ignoring certain values, there is also functionality to add a where clause filter to further control the denominator used in a certain layer.  The function `set_denom_where` is used in these situations.  Throughout a table and it's layers multiple where clauses can be used to filter data different ways.  These can be used in a variety of different ways to customize how a table is displayed.  The application of these where clauses to the denominator data can be controlled using `set_denom_where`.

The `set_denom_where` function is used to specify any additional filtering of the denominator data.  This is done on top of any filtering specified in the `tplyr_table` where clause.  The default for `set_denom_where` is the layer level where clause if one is supplied.  This default can be overwritten with the value of TRUE if the user would like to not have additional filtering.  While you might think NULL would be used in this case, due to how R processes filtering NULL will not work and a value such as TRUE should be used when the layer level where clause should be ignored.  `set_denom_ignore` can also accept any other value filters in the same form as a where statement would be presented.  These will be passed along when building the layer level denominators.

Take the example shown below.  The first layer has no layer level where clause and no additional where clause set so it will just use denominators calculated using the table label where.  The second layer has a where clause and no additional `set_denom_where` so it will use the table level where and the layer level where when constructing the denominators.  Notice how in this case the percentages in the second layer add up to 100%, this is because the denominator only includes values used in that layer.  The third layer has a layer level where that filters the values used but then uses `set_denom_where` to clear the where clause for the denominators by setting the value to TRUE.  This causes the denominator to include all values and not just those selected for that layer so the percentages will not add up to 100%.
