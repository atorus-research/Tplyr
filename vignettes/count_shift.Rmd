...
Tplyr has helper functions for creating risk differences between treatment groups. The `add_risk_diff()` function adds a risk difference column to the table by using `stats::prop.test()` function in R. The two-way tables are constructed and used in the background. 

Risk differences can be formatted with a call to `set_format_strings()` by naming the argument 'riskdiff'. If both counts and risk differences have custom strings, the count f_str object can be named 'n_counts'.
...
suppressWarnings({
  t <- tplyr_table(adae, TRTA) %>%
    ...
    add_layer(
      group_count(vars(AEBODSYS, AEDECOD)) %>%
        add_risk_diff(c("Placebo", "Treatment")) %>%
        set_format_strings(riskdiff = f_str('xx.xxx, xx.xxx, xx.xxx, xx.xxx, xx.xxx', comp, ref, dif, low, high))
    ) %>%
